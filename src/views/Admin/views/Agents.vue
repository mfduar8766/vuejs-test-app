<template>
  <div>
    <Modal
      :headerText="agentToEdit && agentToEdit ? `Edit Agent: ${fullName}` : ''"
    ...
    >
      <EditAgents
        v-if="agentToEdit"
        :agent="agentToEdit"
        @close="editData($event, false)"
        @editedAgent="editData($event, false)"
      />
    </Modal>
    <Modal
...
export default {
  ...
  computed: {
    fullName() {
      return `${this.agentToEdit.firstName} ${this.agentToEdit.lastName}`;
    }
  },
  watch: {
    searchResult: {
